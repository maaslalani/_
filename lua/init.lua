...
wk.register({K = {("<cmd>lua vim.lsp." .. "buf.hover" .. "()<cr>"), "hover"}, Q = {"<nop>", "nope"}, ["<c-h>"] = {"<c-w>h", "left"}, ["<c-l>"] = {"<c-w>l", "right"}, ["<esc>"] = {("<cmd>" .. "nohl" .. "<cr>"), "nohl"}, g = {d = {("<cmd>lua vim.lsp." .. "buf.definition" .. "()<cr>"), "definition"}, name = __fnl_global__goto, r = {("<cmd>lua vim.lsp." .. "buf.reference" .. "()<cr>"), "reference"}}, gW = {("<cmd>" .. "w !sudo tee % > /dev/null" .. "<cr>"), "suwrite"}}, {mode = "n"})
