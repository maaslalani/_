...
local function autocmd(enter, ft, command)
  return ("autocmd " .. enter .. " " .. ft .. " " .. command .. "\n")
end
return vim.cmd(autocmd("BufEnter", "*.graphql", "set ft=graphql"), autocmd("BufEnter", "*.lock", "set ft=json"), autocmd("BufEnter", "*.nix", "set ft=nix"), autocmd("BufEnter", "*.norg", "hi clear Conceal | set nohlsearch | lua wkneorg()"), autocmd("BufWrite", "*.go", "lua vim.lsp.buf.formatting()"), autocmd("CmdLineEnter", ":", "set nosmartcase"), autocmd("CmdLineLeave", ":", "set smartcase"), autocmd("TermOpen", "*", "setlocal nonumber nocursorline signcolumn=no"), autocmd("TermOpen", "*", "startinsert"))
