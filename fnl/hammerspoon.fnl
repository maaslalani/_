...
(local menu
  {:a {:name :Applications
       ...
       :b {:name :Brave
           :action (launch "Brave Browser")}
       :c {:name :Calendar
           :action (launch :Calendar)}
       :p {:name :1Password
           :action (launch "1Password 7")}
       :r {:name :Reminders
           :action (launch :Reminders)}
       :s {:name :Slack :action (launch :Slack)}
       :t {:name :Tuple :action (launch :Tuple)}
       :n {:name :Notes :action (launch :Notes)}}
   :g {:name :Grid
       :action (fn [] (hs.grid.show))}
   :f {:name :Focus
       :action (fn [] (hs.hints.windowHints))}
   :r {:name :Reload :action hs.reload}
   :s {:name :Search
       :action (fn [] (spoon.HSearch.toggleShow))}
   :w {:name :Window
       :h {:name :Left
           :repeatable true
...
(local start (hs.hotkey.modal.new [:cmd] :space))
